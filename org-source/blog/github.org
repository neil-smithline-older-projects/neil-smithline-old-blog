...
The only problem I foresee is that I run more than just my blog on [[http://nelismithline.com][nelismithline.com]]. But the other applications I run are only for my private use so I can just map them to another sub-domain such as /whatever.neilsmithline.com/. For example, I've been playing with a cloud-based bookmarking system, [[http://www.phoboslab.org/projects/asaph][Asaph]], that allows you to host the bookmarks. You can see it at [[http://www2.neilsmithline.com/asaph/]] if you have an interest in seeing the service in action.
